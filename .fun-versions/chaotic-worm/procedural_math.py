...
class ProceduralSegment(BorderCircle):
    ...
    def project_on_circle(self, next_segment: 'ProceduralSegment') -> None:
        ...
        if abs(angle_diff) > self.max_angle:
            delta = delta.lerp(delta.rotate(math.copysign(self.max_angle - abs(angle_diff), -angle_diff)), 0.2)

        delta = delta.normalize() * next_segment.calculation_radius
...
class ProceduralLimb:
    def __init__(self, segments=[], color=(255, 0, 255, 255), batch=None, debug_draw=False) -> None:
